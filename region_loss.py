...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    for b in range(nB):
        for t in range(50):
            ...
            best_n = -1
            min_dist = 10000
            gx = target[b][t*5+1].float() * nW
            gy = target[b][t*5+2].float() * nH
            gi = int(gx)
            gj = int(gy)
            gw = target[b][t*5+3].float() * nW
            gh = target[b][t*5+4].float() * nH
            gt_box = [0, 0, gw, gh]
            for n in range(nA):
            ...
            iou = bbox_iou(gt_box, pred_box, x1y1x2y2=False) # best_iou need to be improved in pytorch 0.4
